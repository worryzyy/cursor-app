                  ...
                  <div key={platform} className="border-t border-gray-100 pt-3">
                    ...
                    <div className="mb-3 space-y-2">
                      <div className="flex items-start">
                        <span className="mr-2 text-sm font-medium text-gray-700">チェックサム:</span>
...
export async function generateMetadata({ params }: { params: { version: string } }): Promise<Metadata> {
  ...
  if (!version) {
    return {
      title: 'バージョンが存在しません',
      description: '利用可能な他のCursorバージョンをご確認ください',
    }
  }

  return {
    title: `Cursor ${version.version}${version.isLatest ? ' (最新版)' : ''}`,
    description: `Cursor ${version.version}をダウンロード - Windows、macOSおよびLinuxプラットフォーム対応のスマートプログラミングツール`,
    keywords: ['Cursor', version.version, 'Cursor 履歴バージョンダウンロード', 'プログラミングツール', 'AIエディター'],
    alternates: {
    ...
    <main className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
          <h2 className="text-2xl font-bold mb-4">バージョン詳細</h2>
          <p>
            このページでは、Cursor バージョン {version} に関する詳細情報を提供しています。以下で、さまざまなオペレーティングシステムおよびアーキテクチャ向けのダウンロードリンクを見つけることができます。各ダウンロードには、お使いのシステムに適切なインストーラーパッケージが含まれています。
          </p>
        </div>
      </div>
